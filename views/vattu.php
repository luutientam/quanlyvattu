<head>
    ...
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script> <!-- Liên kết đến thư viện jQuery để hỗ trợ các thao tác JavaScript -->
</head>
    ...
    <main class="content">
        ...
        <form id="searchForm" method="POST">
            <div class="search-bar">
                <input type="text" id="txtTimKiem" placeholder="Tìm kiếm vật tư..." name="txtTimKiem"> <!-- Trường nhập từ khóa tìm kiếm -->
                <button type="button" class="btn-search" id="btnSearch">Tìm kiếm</button> <!-- Nút tìm kiếm -->
                <select name="loai-vat-tu" id="loai-vat-tu"> <!-- Dropdown chọn loại vật tư -->
                    <option value="all">Tất cả loại vật tư</option> <!-- Tùy chọn tất cả loại vật tư -->
                    <?php foreach ($dataLoaiVatTu['data'] as $loai) { ?> <!-- Lặp qua các loại vật tư -->
        ...
        <table class="table">
            ...
            <tbody id="vatTuTableBody">
                <?php foreach ($data['data'] as $vatTu) { ?> <!-- Lặp qua từng vật tư và hiển thị trong bảng -->
                    <tr>
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    `;
                        ...
                        vatTuTableBody.innerHTML = '<tr><td colspan="10">Không tìm thấy vật tư</td></tr>'; // Hiển thị khi không tìm thấy vật tư
                    }
                })
            ...
            fetch("http://localhost/quanlyvattu/controllers/VatTu_api.php", {
                ...
                .then(data => {
                    if (data.status === 200) { // Kiểm tra xem yêu cầu có thành công không
                        ...
                        window.location.href = "http://localhost/quanlyvattu/index.php"; // Chuyển hướng trang sau khi cập nhật
                    } else {
                    ...
                    fetch("http://localhost/quanlyvattu/controllers/VatTu_api.php", {
                        ...
                        .then(data => {
                            if (data.status === 200) { // Kiểm tra nếu yêu cầu thành công
                                ...
                                window.location.href = "http://localhost/quanlyvattu/index.php"; // Chuyển hướng trang sau khi xóa
                            } else {
                                alert(data.message); // Thông báo lỗi nếu yêu cầu không thành công
                            }
                        })
